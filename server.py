...
for net in nets:
    if net.ssid == 'vodafoneB038':  # DH: SSID y Password
        wlan.connect(net.ssid, auth=(net.sec, '8JEZR9R9CK9YCK'), timeout=5000)
        print("Conectado")
        while not wlan.isconnected():
            machine.idle()
        break # DH: Fin de conexión
...
class Server:
 ...
 def __init__(self, port = 80):
     ...
     self.port = port
     self.www_dir = WEB_PAGES_HOME_DIR # Directory where webpage files are stored
 ...
 def _wait_for_connections(self):
     ...
     while True:
         ...
         treqhead = treq.split("\r\n\r\n")[0]
         treqbody = treq[len(treqhead):].lstrip()
